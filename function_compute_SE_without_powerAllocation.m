function SE = function_compute_SE_without_powerAllocation( rho ,signal, interference,prelog_factor)
...
for j = 1:L
    ...
    for k = 1:K
        ...
        SE(k,j) = prelog_factor * log2(1+(rho * signal(k,j)/sum(sum(rho .* interference(:,:,k,j)))+1));
    end
end
end